...
Istnieje abstrakcyjna definicja algebr von Neumanna, bez odwzoływania się
...
uogólnienia kryterium Peresa-Horodeckich na szeroką klase iniektywnych alebr von Neumanna.
...
\end{Definition}
%Wprowadźmy relację częściowego porządku w zbiorze projektorów algebry $\mathfrak{M}$
%i napiszmy $p \preceq q$,
%jeżeli $p$ jest równoważne projektorowi $q_{1} \leq q$.
...
Jest jasne, że projektor przemienny jest skończony, a projetor minimalny jest przemienny.
...
\begin{Definition}
    ...
    Jeżeli $\mathbf{1} \in \mathfrak{M}$ jest projetorem skończonym i
    ...
    projetorów, wówczas mówimy, że $\mathfrak{M}$ jest \emph{typu II$_{\infty}$}.
\end{Definition}

\begin{Theorem}
    \label{thm:decompositionofvNalg}
    Każda algebra von Neumanna $\mathfrak{M}$ rozkłada się w sposób
    jednoznaczy na sumę prostą algebr typu I, II$_{1}$, II$_{\infty}$ oraz typu III.
